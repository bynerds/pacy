    ...
    var body: some View {
        TimelineView(MetricsTimelineSchedule(from: workoutManager.builder?.startDate ?? Date(),
            ...
            VStack(alignment: .leading) {
                ...
                HStack(alignment: .firstTextBaseline) {
                    Text("HR \(String(format: "%.0f%%", (workoutManager.averageHeartRate / 200) * 100))")
                        .font(.system(size: 30, weight: .regular, design: .rounded).monospacedDigit().lowercaseSmallCaps())
                        .padding(.vertical, -4)  // Adjust padding to reduce vertical space
                }
                Text("HR \(workoutManager.heartRate.formatted(.number.precision(.fractionLength(0))))")
                    .font(.system(size: 30, weight: .regular, design: .rounded).monospacedDigit().lowercaseSmallCaps())
                    .padding(.vertical, -4)  // Adjust padding to reduce vertical space
                Text(String(format: "%.1f KM", workoutManager.distance / 1000))
                    .font(.system(size: 30, weight: .regular, design: .rounded).monospacedDigit().lowercaseSmallCaps())
                    .padding(.vertical, -4)  // Adjust padding to reduce vertical space
            }
...
struct AverageDurationView: View {  // Same name as before for Avg Pace Last 3km
    @EnvironmentObject var workoutManager: WorkoutManager
    ...
    var body: some View {
        TimelineView(MetricsTimelineSchedule(from: workoutManager.builder?.startDate ?? Date(),
                                             isPaused: workoutManager.session?.state == .paused)) { context in
            VStack(alignment: .leading) {
                Text("Avg Pace Last 3km")  // Corrected label
                    .font(.system(size: 17, weight: .medium, design: .rounded))
    ...
    var body: some View {
        TimelineView(MetricsTimelineSchedule(from: workoutManager.builder?.startDate ?? Date(),
            ...
            VStack(alignment: .leading) {
                ...
                Text("\(String(format: "%.0f%%", (workoutManager.averageHeartRate / 200) * 100))")
                    .font(.system(size: 46, weight: .regular, design: .default).monospacedDigit().lowercaseSmallCaps())
            }
            .frame(maxWidth: .infinity, alignment: .leading)
            .ignoresSafeArea(edges: .bottom)
